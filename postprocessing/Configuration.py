...
class Configuration(object):
    ...
    def __init__(self, config_file):
        ...
        # Catalog AMQ queues
        self.catalog_data_ready = config[
            'catalog_data_ready'] if 'catalog_data_ready' in config else 'CATALOG.DATA_READY'
        ...
        self.reduction_data_ready = config[
            'reduction_data_ready'] if 'reduction_data_ready' in config else 'REDUCTION.DATA_READY'
        ...
        self.heartbeat_ping = config[
            'heartbeat_ping'] if 'heartbeat_ping' in config else '/topic/SNS.COMMON.STATUS.PING'
        # Reduction catalog AMQ queues
        self.reduction_catalog_data_ready = config[
            'reduction_catalog_data_ready'] if 'reduction_catalog_data_ready' in config else 'REDUCTION_CATALOG.DATA_READY'
        ...
        self.create_reduction_script = config[
            'create_reduction_script'] if 'create_reduction_script' in config else 'REDUCTION.CREATE_SCRIPT'
        self.service_status = config[
            'service_status'] if 'service_status' in config else "/topic/SNS.${instrument}.STATUS.POSTPROCESS"
        ...
        self.python_dir = config['python_dir'] if 'python_dir' in config else os.path.join(self.sw_dir,
                                                                                           'postprocessing')
        self.remote_script = config['remote_script'] if 'remote_script' in config else os.path.join(self.sw_dir,
                                                                                                    'scripts',
                                                                                                    'remoteJob.sh')
        ...
        self.wait_notification_period = config[
            'wait_notification_period'] if 'wait_notification_period' in config else 900

        self.web_monitor_url = config[
            'webmon_url_template'] if 'webmon_url_template' in config\
            else "https://monitor.sns.gov/files/$instrument/$run_number/submit_reduced/"
        ...
        if isinstance(self.processors, list):
            for p in self.processors:
                ...
                if len(toks) == 2:
                    ...
                    processor_module = __import__("postprocessing.processors.%s" % toks[0], globals(), locals(),
                                                  [toks[1], ], -1)
                    try:
    ...
    def log_configuration(self, logger=logging):
        ...
        if self.remote_execution:
            ...
            logger.info("    Max chunk memory: %s", self.max_memory)
            logger.info("    Max nodes: %s", self.max_nodes)
        else:
        ...
        logger.info("  - Image posting URL: %s", self.web_monitor_url)
...
def read_configuration(config_file=None, defaults=[CONFIG_FILE, CONFIG_FILE_ALTERNATE]):
    ...
    if config_file is None:
        for config_default in defaults:
            ...
            raise RuntimeError("Default configuration file(s) do not exist, or unreadable: %s" % str(defaults))
