...
def do_reduction(path, outdir):
    ...
    default_reduction_files = glob.glob(
        os.path.join("/SNS/HYS/shared/templates/reduce_HYS_*.py")
    )
    ...
    if reduction_files != []:
        ...
        if os.path.getmtime(latest_reduction) < os.path.getmtime(
            latest_default_reduction
        ):
    ...
    proc = subprocess.Popen(
        cmd, shell=True, stdin=subprocess.PIPE, universal_newlines=True, cwd=outdir
    )
    proc.communicate()
